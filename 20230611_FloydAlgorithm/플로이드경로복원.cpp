...
// 3에서 5로 가는 경로를 찾으려면 nxtTable[3][5]를 조회하여 3에서 이동해야 하는 노드를 구한다. (즉, 3에서 1로 이동해야 한다.)
// 계속하여 1에서 5로 가는 경로를 찾기 위해 nxtTable[1][5]를 조회하여 1에서 이동해야 하는 노드, 4를 구한다.
// 4에서 5로 가는 경로를 찾기 위해 nxtTable[4][5]를 조히하여 4에서 이동해야 하는 노드, 5를 구한다.
// 이 노드는 도착 노드이므로 전체 경로 복원을 종료한다. (3에서 5로 가는 경로는 3 -> 1 -> 4 -> 5이다.)
