...
vector<int> reflectedSquaresOf(int square) {
    vector<int> result;
    int row = rowNumOf(square);
    int col = colNumOf(square);
    int nRow = boardSize+1-row;
    int nCol = boardSize+1-col;
    result.push_back(makeSquare(nRow, col)); //horizontal reflection
    result.push_back(makeSquare(row, nCol)); //vertical reflection
    result.push_back(makeSquare(nRow, nCol)); //point reflection
    result.push_back(makeSquare(col, row)); //diagonal reflection
    result.push_back(makeSquare(col, nRow)); //diagonal reflection of horizontal reflection
    result.push_back(makeSquare(nCol, row)); //diagonal reflection of vertical reflection
    result.push_back(makeSquare(nCol, nRow)); //diagonal reflection of point reflection
    return result;
}
 ...
 * 두 맵을 합한 결과를 반환한다.
 */
unordered_map<int, double> mergeMaps(unordered_map<int, double> m0, unordered_map<int, double> m1) {
    unordered_map<int, double> result;
    for (auto entry:m0) {
 ...
 *  맵 목록을 합한 결과를 반환한다,
 */
unordered_map<int, double> mergeMaps(vector<unordered_map<int, double>> ms) {
    unordered_map<int, double> result;
...
unordered_map<int, double> getDpArrayOf(int square, int steps) {
    ...
    //점화식 적용 전 대칭되는 칸을 먼저 조회해 본다.
    for (int reflectedSquare:reflectedSquaresOf(square)) {
        if (dpArrayFixed[reflectedSquare][steps]) {
            dpArray[square][steps] = dpArray[reflectedSquare][steps];
            dpArrayFixed[square][steps] = true;
            return dpArray[square][steps];
        }
    } //reflectedSquare loop

    //점화식
    for (auto entry:getDpArrayOf(square, 1)) { //entry.first는 square에서 출발하여 나이트가 갈 수 있는 칸이 된다.
        dpArray[square][steps] = mergeMaps(dpArray[square][steps], getDpArrayOf(entry.first, steps-1));
    }
    dpArrayFixed[square][steps] = true;
    return dpArray[square][steps];
}
